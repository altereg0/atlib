...
void SoftwareUART::begin(uint32_t baudrate) {
  bit_delay = (F_CPU / baudrate) / 4;
  Digital(tx_pin).high();
}

void SoftwareUART::write(uint8_t data) {
...
uint8_t SoftwareUART::read(void) {
#warning "SoftwareUART::read is not imeplemented, yet!"
  return (0);
}
